...
function DragAndDrop() {
    ...
    const handleSubmit = async (url,nodeId) => {
        try{
            const response = await axios.get('https://drag-and-drop-webscraper-with-llm.onrender.comscrapper',{params: { url }})
    ...
    const initialNodes = [
      { id:'1', type: 'webScrapper', data: { label: 'Web Scrapper', image: "./assets/blackweb.svg", handleSubmit } },
      { id:'2', type: 'summary', data: { label: 'Summary', image: "./assets/black.svg" } },
    ];
...
return (
        <>
            <div className="nodes">
                <img id='cloud' src="./assets/cloud.svg"/>
                {initialNodes.map((node)=>{
                    return <Node key={node.id} id={node.id} logo={node.data.image === "./assets/blackweb.svg" ?  "./assets/web.svg" : "./assets/summary.svg"} name={node.data.label}/>
                })}
            </div>
